...
**PoisonedRAG** introduces an attack on Retrieval-Augmented Generation (RAG) systems, where an adversary injects malicious texts into the system's knowledge base in order to manipulate the outputs of large language models (LLMs). The aim is to cause the LLM to produce a specific, attacker-chosen answer in response to a target question, without requiring any modification of the LLM itself.
