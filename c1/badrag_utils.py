...
class BadRAG:
    ...
    def __init__(
        ...
        # Get word embedding weights and register gradient hook
        ...
        # Load passage embeddings and corresponding IDs
    ...
    def generate_poison(
        ...
        Returns:
            Tuple[Tensor, int]: Final adversarial token IDs (1 x sequence_length) and number of iterations used.
        """
        mask_id = self.tokenizer.mask_token_id or self.tokenizer.unk_token_id

        # Initialise poisoned sequence with [MASK] tokens
        ...
        for t in range(T):
            ...
            with torch.no_grad():
                # Select a random position and candidate replacement tokens
            ...
            # Custom metric: penalise if clean queries are too similar
